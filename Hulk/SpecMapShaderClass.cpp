...
{
	if (!InitializeShader(device, hwnd, "Data/Shaders/SpecMapShader.fx"))
		return false;

	return true;
}

bool SpecMapShaderClass::InitializeShader(ID3D10Device *device, HWND hwnd, char *shaderFilename)
{
	HRESULT result;
...
void SpecMapShaderClass::OutputErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char *shaderFilename)
{
	char* compileErrors;
...
void SpecMapShaderClass::Render(ID3D10Device* device, unsigned long indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D10ShaderResourceView** texturesArray,
	D3DXVECTOR3 cameraPosition, D3DXVECTOR3 lightDirection, D3DXCOLOR diffuseColor, D3DXCOLOR ambientColor, D3DXCOLOR specularColor, float specularPower)
{
	SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texturesArray, cameraPosition, lightDirection, diffuseColor, ambientColor, specularColor, specularPower);
...
void SpecMapShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D10ShaderResourceView** texturesArray,
	D3DXVECTOR3 cameraPosition, D3DXVECTOR3 lightDirection, D3DXCOLOR diffuseColor, D3DXCOLOR ambientColor, D3DXCOLOR specularColor, float specularPower)
{
	m_worldMatrixPtr->SetMatrix((float*)&worldMatrix);
	m_viewMatrixPtr->SetMatrix((float*)&viewMatrix);
	m_projectionMatrixPtr->SetMatrix((float*)&projectionMatrix);
	m_texturesArrayPtr->SetResourceArray(texturesArray, 0, 3);
...
void SpecMapShaderClass::RenderShader(ID3D10Device* device, unsigned long indexCount)
{
